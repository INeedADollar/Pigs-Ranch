...
void PigsBox::showDetailsWindow() {
    ...
    d->setBoxName(boxName);
    d->setBoxHallName(boxHallName);

    d->setBoxType(boxType());
    d->setPigsNumber(pigsNumber);

    d->setPopulationDate(populationDate);
    d->setReleaseDate(releaseDate);

    d->setBoxDimensions(boxSize());
    d->show();
}
/**************************************************************/
