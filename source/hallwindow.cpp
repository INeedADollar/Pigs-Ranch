...
void HallBluePrintWidget::resizeBoxes(){
    ...
    for(PigsBox* box : boxes) {
        if(box->boxType().getBoxType() == PigsBoxType::LittleBox) {
            QSize littleBoxSize = QSize(0.125 * bluePrintSize.height(), 0.25 * bluePrintSize.height());
            box->setRect(QRect(currentPoint, littleBoxSize));

            currentPoint = QPoint(currentPoint.x() + littleBoxSize.width() + 4, currentPoint.y());
        }

        if(box->boxType().getBoxType() == PigsBoxType::NormalBox) {
            QSize littleBoxSize = QSize(0.5 * bluePrintSize.height(), 0.25 * bluePrintSize.height());
            box->setRect(QRect(currentPoint, littleBoxSize));
        ...
        if(currentPoint.x() > bluePrintSize.width() && currentPoint.y() == 4)
            currentPoint = QPoint(4, height() - 4 - 0.25 * height());
    }
}
...
void HallBluePrintWidget::mouseClickEvent(QMouseEvent *event){
