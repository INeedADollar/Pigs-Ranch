...
void HallsWindow::addHall(PigsHall *hall){
    if(ui->listWidget->count() && ui->listWidget->item(0)->text() == "No hall added")
...
void HallsWindow::on_pushButton_3_clicked() {
    if(ranch->pigsHalls().isEmpty()){
        InfoDialog *d = new InfoDialog;
        d->setWindowTitle("Ranch empty");
        d->setDisplayText("Ranch is empty. Please create at least a hall to save the ranch!");
        d->show();
        return;
    }

    QString filePath = QFileDialog::getSaveFileName(this, "Save ranch", "", "Json Document (*.json)");
    if(filePath != ""){
        QFile file(filePath);
...
void HallsWindow::on_pushButton_2_clicked(){
    QString filePath = QFileDialog::getOpenFileName(this, "Load ranch", "", "Json Document (*.json)");
    if(filePath != ""){
        QFile file(filePath);
        if(!file.open(QIODevice::ReadOnly)){
            ...
            if(ranch->loadRanchFromJson(document)) {
                ui->listWidget->clear();
                addHallsToList();

                InfoDialog *d = new InfoDialog;
                d->setWindowTitle("Success");
                d->setDisplayText("File loaded succesfully!");
                d->show();
            }
            else {
                ...
                InfoDialog *d = new InfoDialog;
                d->setWindowTitle("Invalid file");
                d->setDisplayText("Selected file is not a valid json file for this app!");
                d->show();
            }
        }
    }
